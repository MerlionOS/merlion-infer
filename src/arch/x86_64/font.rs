...
/// Classic VGA/BIOS compatible font covering ASCII 32-126.
/// Each glyph is 16 bytes (16 rows, 8 pixels wide, MSB = leftmost pixel).

pub const CHAR_WIDTH: usize = 8;
pub const CHAR_HEIGHT: usize = 16;

/// Returns the 16-byte glyph bitmap for an ASCII character.
/// Non-printable or out-of-range characters return a solid block.
pub fn glyph(c: u8) -> &'static [u8; 16] {
    if c >= 32 && c < 127 {
        &FONT_DATA[c as usize - 32]
    } else {
        &FALLBACK_GLYPH
    }
}

/// Solid block glyph for unknown characters.
static FALLBACK_GLYPH: [u8; 16] = [0xFF; 16];

/// Font data for ASCII 32 (space) through 126 (~). 95 glyphs × 16 bytes.
